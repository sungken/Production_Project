...
//
//		try {
//			BufferedWriter writer = new BufferedWriter(new FileWriter(Path.CHECKASSIGNORDER));
//
//			for (Assign assignOrder : AssignData.assignList) {
//				//System.out.println("[주문서 번호]\t[구역]\t[대리점명]\t[모델명]\t[수량]\t[상태]");
//				String line = String.format("%s■%s■%s■%s■%d■%s■%s\r\n",
//						assignOrder.getId(),
//						assignOrder.getArea(), assignOrder.getAgencyName(),
//						assignOrder.getModelId(),assignOrder.getQuantity(),
//						assignOrder.getState(),assignOrder.getUserId());
//
//				writer.write(line);
//
//			}
